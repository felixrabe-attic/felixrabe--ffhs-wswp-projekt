...
Soweit die Idee. Nun das selbe mit der gesamten Tabelle, welche insgesamt \Sexpr{f(nrow(usnames))} Einträge umfasst:
...
\begin{align*}
  \Omega      &= \text{Ergebnisraum, Weltbevölkerung (jede Person ist ein Elementarereignis)} \\
  |\Omega|    &= \Sexpr{f(prob_omega)} \\
  B           &= \text{Jemand ist Amerikaner} \\
  |B|         &= \Sexpr{f(prob_b)} \\
  P(B)        &= \frac{|B|}{|\Omega|} = \Sexpr{prob_b / prob_omega} \\
  A           &= \text{Jemand heisst ``Smith''} \\
  A \cap B    &= \text{Jemand heisst ``Smith'' und ist Amerikaner} \\
  |A \cap B|  &= \Sexpr{f(prob_a_cap_b)} \\
  P(A \cap B) &= \frac{|A \cap B|}{|\Omega|} = \Sexpr{prob_a_cap_b / prob_omega} \\
  A|B         &= \text{Ein Amerikaner heisst ``Smith''} \\
  P(A|B)      &= \frac{P(A \cap B)}{P(B)} = \Sexpr{prob_a_cap_b / prob_b} \\
  P(A \cap B) &= \text{Jemand heisst ``Smith'' und ist Amerikaner}
\end{align*}

Da die Ereignisse $A$ und $B$ abhängig sind ($A$ unterscheidet sich je nach Land), gilt nicht (zwingend): $P(A \cap B) = P(A) \cdot P(B)$.
