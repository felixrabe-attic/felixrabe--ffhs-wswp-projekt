...
Zunächst wird überprüft, wie stark Jahr und Bevölkerung miteinander in Zusammenhang stehen. Das verdeutlicht die Korrelationsrechung.\footnote{Die konkreten Werte, die für die folgenden Berechnungen verwendet werden, sind in Tabelle ~\ref{population_total_table} auf Seite ~\pageref{population_total_table} aufgelistet.}
