...
\begin{align*}
  ...
  \sqrt{\frac{\hat{p} (1 - \hat{p})}{n}} &= \sqrt{\frac{\Sexpr{p_x * (1 - p_x)}}{\Sexpr{n}}} = \sqrt{\Sexpr{p_x * (1 - p_x) / n}} = \Sexpr{sqrt_p} \\
  c       &= \frac{k}{\sqrt{\frac{\hat{p} (1 - \hat{p})}{n}}} = \frac{\Sexpr{k}}{\Sexpr{sqrt_p}} = \Sexpr{k / sqrt_p}
\end{align*}
