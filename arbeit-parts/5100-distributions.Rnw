...
Eine Google-Anfrage nach ``anteil raucher in der schweiz'' ergibt eine NZZ-Schlagzeile mit der Zahl 27\%.\footnote{http://www.nzz.ch/aktuell/startseite/27-prozent-der-schweizer-bevoelkerung-raucht-1.5968845}
...
\begin{align*}
  ...
  k       &= p - \hat{p} = \frac{|A| - X}{n} = \frac{\Sexpr{a - x}}{\Sexpr{n}} = \Sexpr{(a-x) / n}
\end{align*}
